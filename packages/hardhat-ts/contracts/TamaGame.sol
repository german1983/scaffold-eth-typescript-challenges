...
contract TamaGame {
  ...
  bool public hungry;
  uint256 foodCounter = block.timestamp + 6 hours;
  uint256 hungryCounter = block.timestamp + 12 hours;
  ...
  function feed() public returns (uint256) {
    uint256 oldXP = characters[msg.sender].getXP();
    uint256 newXP = oldXP.add(2);
  ...
  function upgrade1() public returns (uint256) {
